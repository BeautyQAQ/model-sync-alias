...
var service = pluginservice.NewService(hostLog)
...
func cliproxyPluginCall(method *C.char, request *C.uint8_t, requestLen C.size_t, response *C.cliproxy_buffer) C.int {
	...
	if method == nil {
		writeResponse(response, pluginservice.ErrorEnvelope("invalid_method", "method is required"))
		return 1
	}
	...
	raw, errHandle := service.HandleMethod(C.GoString(method), requestBytes)
	if errHandle != nil {
		writeResponse(response, pluginservice.ErrorEnvelope("plugin_error", errHandle.Error()))
		return 1
	}
