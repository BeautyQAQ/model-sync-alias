...
func buildDesiredModels(ids []string, existing []sdkconfig.OpenAICompatibilityModel, cfg settings) []sdkconfig.OpenAICompatibilityModel {
	...
	for _, id := range ids {
		...
		model := oldByName[key]
		model.Name = id
		// Keep every synchronized model on CPA's explicit alias path. Standalone
		// models use a self-alias; normalized variants use their canonical alias.
		model.Alias = canonicalModelName(id, cfg)
